...
func TestUseCase_Approve(t *testing.T) {
	type approveMockBehavior func(mr *Mockrepo, id int64)
	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(mr *Mockrepo, id int64) {
				mr.EXPECT().ChangeStatus(gomock.Any(), id, advertentity.StatusInitial, advertentity.StatusActive).Return(
					advertentity.Advert{ID: 1, Status: advertentity.StatusActive}, nil)
			},
			expectedID: 1,
		},
		{
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			advert, err := uc.Approve(context.Background(), testCase.inputID)
