...
CREATE TABLE advertapp.users(
    ...
    CONSTRAINT email_len_valid          CHECK(length(trim(email)) BETWEEN 3 and 255),
    CONSTRAINT email_lowercase          CHECK(email = lower(email)),
    CONSTRAINT full_name_len_valid      CHECK(length(trim(full_name)) BETWEEN 3 AND 100),
    CONSTRAINT phone_number             CHECK(phone_number ~ '^\+[1-9]\d{1,14}$'),
    CONSTRAINT phone_number_len         CHECK(length(trim(phone_number)) BETWEEN 4 AND 20),
    CONSTRAINT user_role_len_valid      CHECK(length(trim(role)) BETWEEN 1 AND 100),
